...
- `bun run check-node` builds `dist/`, then runs `scripts/reload-node.mjs` under `node`: it loads a `rafa.config.mjs` in a `mkdtemp` directory with `reload: true`, rewrites it from `{ port: 8000 }` to `{ port: 9000 }` and loads again, then repeats with `reload` off in a second directory as the control. It exits 0 only when the reloading second load reads `9000` and the control's reads `8000`, and 1 naming each mismatch otherwise. It needs `node` on `PATH`; CI runs it after `build`, under Node LTS. The script loads `dist/index.js` with `import()` at run time, not a static import, because lint runs over source before any build; run without a build it exits 1 saying so.
